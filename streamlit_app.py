...
st.markdown("<p style='text-align: center; font-size: 22px;'>Developed by H. RJ Nikzat</p>", unsafe_allow_html=True)
...
def clean_fringes(ipwa, ipwoa):
    ...
    for i in range(ipwa.shape[1]):
        ...
        # Skip PCA if too few samples
        if col.shape[0] < 2 or col.shape[1] < 1:
            continue
...
def process(bytes_data, name):
    try:
        ...
        # AUTO-FIX for common .ibw formats
        if w.ndim == 3:
            if w.shape[2] >= 2:
                ipwa = w[:, :, 0].T
                ipwoa = w[:, :, 1].T
...
if uploaded:
    ...
    with zipfile.ZipFile(zip_buf, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        if success:
            zip_buf.seek(0)
            st.success(f"Processed {success} file(s)!")
            st.download_button("📦 Download All", zip_buf, "FringeFree_ColdAtoms.zip", "application/zip")
            st.balloons()
        else:
            st.error("No files worked. Make sure your .ibw files have paired atom + reference images.")
